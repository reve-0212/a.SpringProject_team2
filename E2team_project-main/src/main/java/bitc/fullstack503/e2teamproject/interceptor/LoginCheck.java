  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
    ...
    if (requestURI.startsWith("/pro")) {
      if (userId == null) {
        ...
        sendAlert(response, "로그인이 필요합니다.", "/reply/write");
        sendAlert(response, "로그인이 필요합니다.", "/review/write");
        return false;
      }
    }
...
// 3. 댓글 작성 기능 로그인 체크
    if (requestURI.startsWith("/reply/write")) {
      if (userId == null) {
        sendAlert(response, "로그인이 필요합니다.", "/user/");
        return false;
      }
    ...
    // 4. 리뷰 작성 기능 로그인 체크
    if (requestURI.startsWith("/review/write")) {
      if (userId == null) {
        sendAlert(response, "로그인이 필요합니다.", "/user/");
        return false;
      }
